...
| `evaluator_config.py` | 75 | evaluator repetition and inline runtime normalization shared by config, workspace, and execution |
...
| `evaluation/identity.py` | 230 | canonical task-set identity plus checkout and frozen-baseline comparability |
...
| `preflight/checks.py` | 125 | exact-environment host tool and candidate dependency-lock probes |
...
| `runtime_config.py` | 360 | inline runtime validation, endpoint identity, canonical resolution, and trusted loading |
...
Total `src/evolve/` budget: **13085 lines**. The budget admits the explicit content-backed
