"""Lifecycle budgets for a retrying Harbor mutation operator.
...
def uses_harbor_per_attempt_timeout(config: Mapping[str, object]) -> bool:
    """Whether a mutation config receives Harbor's per-attempt timeout."""

    return config.get("runner") == "harbor" and harbor_agent_supports_per_attempt_timeout(config.get("agent"))


@dataclass(frozen=True)
class HarborMutateBudget:
    """Calculated nested and outer deadlines for one Harbor mutation run."""
...
def harbor_mutate_budget(
    agent_timeout_s: float,
    max_retries: int,
) -> HarborMutateBudget:
    """Return a normalized lifecycle budget."""

    return HarborMutateBudget(
