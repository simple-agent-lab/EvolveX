...
def _check_digest(engine: str) -> Check:
    ...
    if engine == "harbor":
        return Check(
            "runtime digest",
            "fail",
            "EVOLVE_RUNTIME_DIGEST is not set; export the immutable evaluator "
            "runtime digest before `evolve init`",
        )
...
def _check_dataset(evaluator: dict[str, Any], *, rollout_needs_dataset: bool) -> Check:
    ...
    if invalid:
        return Check(
            "dataset",
            "warn",
            f"{resolved}: {len(valid)} valid tasks; Harbor will silently skip "
            f"{', '.join(invalid[:5])}",
        )
