...
def invoke_operator(
    ...
    *,
    ...
    if reserved_overrides:
        raise RuntimeError(
            "operator invocation cannot replace implementation keys: " + ", ".join(reserved_overrides)
        )
...
def _assert_operator_prerequisites(name: str, run_dir: Path, configured: dict[str, Any]) -> None:
    ...
    for prerequisite in prerequisites:
        ...
        if error is not None:
            raise RuntimeError(
                f"operator {name} requires valid {prerequisite} output: {_operator_output_note(error)}"
            )
...
def _invocation_checkout(
    ...
    if parent is not None:
        _assert_child_worktree_for_parent(workspace, selected, parent)
...
def finalize_child(workspace: Path, genid: str, *, parent: str | None = None) -> bool:
    ...
    with workspace_run_lock(workspace):
        ...
        if not _run_gate_and_record(workspace, exp_id, genid, resolved_parent, operator_blocks(workspace)):
            raise RuntimeError(
                f"generation {genid} gate failed; fix the operator or runtime, then retry finalize"
            )
        return True
...
def commit_agent_child(workspace: Path, child_worktree: Path, parent: str, genid: str) -> None:
    ...
    with workspace_run_lock(workspace):
        ...
        include, exclude = surface_patterns(workspace)
        candidate = build_candidate_snapshot(child_worktree, f"gen/{parent}", include=include, exclude=exclude)
