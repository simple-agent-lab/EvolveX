...
`trace_analyzer` converts method-neutral rollout cases into bounded evidence for
`meta_agent`. It is optional, but recipes that need structured observations
should configure it explicitly.

```text
rollout/cases.json
  → trace_analyzer
  → trace_analyzer/evidence/
...
```python
class TraceAnalyzerOperator:
    def analyze(self, checkout, ctx) -> TraceAnalyzerResult: ...
```
...
```text
runs/gen-N/trace_analyzer/evidence/raw_traces.jsonl
runs/gen-N/trace_analyzer/evidence/failure_records.json
runs/gen-N/trace_analyzer/evidence/failure_patterns.json
runs/gen-N/trace_analyzer/evidence/passing_behaviors.json
runs/gen-N/trace_analyzer/evidence/reflective_records.jsonl
runs/gen-N/trace_analyzer/evidence/metrics.json
runs/gen-N/trace_analyzer/evidence/manifest.json
runs/gen-N/trace_analyzer/evidence/selected.md
```
