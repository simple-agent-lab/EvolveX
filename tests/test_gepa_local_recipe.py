...
def test_eval_stub_targets_the_resolved_split(tmp_path: Path) -> None:
    ...
    result = run_evolve("init", str(workspace), "--recipe", "gepa_local", "--dataset", str(TASKS_LOCAL))
...
def test_gepa_local_full_generation_improves_champion(tmp_path: Path) -> None:
    ...
    knowledge_path.write_text(
        knowledge_path.read_text() + "".join(f"- {question} = {answer}\n" for question, answer in facts.items())
    )
    ...
    assert result.returncode == 0, result.stderr
    comparison = json.loads((workspace / "runs" / "gen-1" / "validate" / "comparison.json").read_text())
    ...
    assert run_evolve("commit", str(workspace), str(child), "--parent", "0", "--genid", "1").returncode == 0
