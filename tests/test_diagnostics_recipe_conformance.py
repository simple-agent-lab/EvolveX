...
def test_partner_recipes_share_receipt_feedback_sdk_and_report_diagnostics(
    ...
    init_workspace(
        InitOptions(workspace=workspace, recipe=recipe, seed=str(seed), dataset=str(dataset))
    )
    ...
    feedback_payload = json.loads(
        (run_dir / "feedback" / "evaluation_diagnostics.json").read_text()
    )[0]
