...
def test_ahe_recipe_runs_train_rollout_trace_analysis_meta_agent_and_gate(tmp_path: Path) -> None:
    ...
    assert {
        ...
        for name in ("select", "rollout", "trace_analyzer", "meta_agent", "gate", "record")
    } == {
        "select": 600,
        "rollout": 3600,
        "trace_analyzer": 600,
    ...
    baseline_args_path = next((workspace / "runs" / "evaluations" / "genesis").rglob("jobs/args.json"))
    baseline_args = json.loads(baseline_args_path.read_text())
    ...
    assert any(event.get("kind") == "genesis_eval" for event in events)
