...
def test_driver_and_doctor_preserve_active_agent_edits(tmp_path: Path) -> None:
    workspace, evolve_home = init_workspace(tmp_path)
    _certify_baseline(workspace, evolve_home)
    ...
    doctor = run_evolve("doctor", str(workspace), env={"EVOLVE_HOME": str(evolve_home)})
    assert doctor.returncode == 0, doctor.stderr
    assert "preserved dirty worktree gen-1" in doctor.stdout
    assert "irreplaceable" in target.read_text()
...
def test_doctor_removes_only_clean_stale_worktree(tmp_path: Path) -> None:
    workspace, evolve_home = init_workspace(tmp_path)
    _certify_baseline(workspace, evolve_home)
    child = workspace / "runs/worktrees/gen-1"
    assert run_evolve("fork", str(workspace), "0", str(child)).returncode == 0
    ...
    assert doctor.returncode == 0, doctor.stderr
    assert "removed stale worktree gen-1" in doctor.stdout
    assert not child.exists()
...
def test_driver_detects_agent_worktree_outside_managed_directory(tmp_path: Path) -> None:
    ...
    assert doctor.returncode == 0, doctor.stderr
    assert "preserved linked worktree outside runs/worktrees" in doctor.stdout
    assert child.exists()
