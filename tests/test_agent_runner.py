...
from evolve.agent import AgentCommandError, _timeout_float, run_mutate
...
def test_run_mutate_runs_command_in_workspace_with_prompt_file(tmp_path: Path) -> None:
...
def test_run_mutate_uses_nested_mutate_command(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    script = tmp_path / "agent.py"
...
def test_run_mutate_applies_caller_owned_environment_overrides(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
    script = tmp_path / "agent.py"
    script.write_text(
...
def test_run_mutate_uses_env_command_and_reports_missing_command(
...
def test_run_mutate_timeout_kills_command_group(tmp_path: Path) -> None:
    workspace = tmp_path / "workspace"
    workspace.mkdir()
