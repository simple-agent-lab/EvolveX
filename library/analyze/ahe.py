...
def _positive_timeout(config: dict[str, object]) -> None:
    timeout = config.get("timeout_per_task")
...
CONFIG = Config(
    {
        "max_tasks": integer(default=90, minimum=1),
        "max_concurrent": integer(default=16, minimum=1),
        "timeout_per_task": number(),
        "retry_attempts": integer(default=1, minimum=1),
        "debugger": object_field(additional_properties=True),
        "debugger_max_retries": integer(minimum=0),
        "debugger_agent_kwargs": object_field(additional_properties=True),
...
ARTIFACTS = [
    ...
    "analyze/analysis/change_evaluation.json",
    "analyze/evidence/selected.md",
    "analyze/evidence/overview.json",
    "analyze/evidence/cases.jsonl",
]
COLLECTION_LIMIT = 32
MAX_NESTING = 6
EXPANSION_FACTOR = 8
    ...
    for attempt in range(1, attempts + 1):
        try:
            result = run_readonly_agent(
                ...
                runner_ctx,
                output_dir=ctx.run_dir / "analyze" / "debugger" / slug / f"attempt-{attempt}",
...
def _change_evaluation(
    ...
    manifest_path = prior_run / "mutate" / "change_manifest.json"
    try:
...
def _reports(root: Path, results: list[DebuggerResult], archive_analysis: Case) -> tuple[str, list[str]]:
    ...
    for result in results:
        ...
        relative = f"analyze/analysis/detail/{_safe_task_name(job.task_name)}.md"
...
class AheAnalyze(AnalyzeOperator):
    def analyze(self, checkout: Path, ctx: OperatorContext) -> AnalyzeResult:
        ...
        return AnalyzeResult(summary=summary, artifacts=[*ARTIFACTS, *detail_artifacts])


if __name__ == "__main__":
    sdk.main(AheAnalyze, config_schema=CONFIG)
