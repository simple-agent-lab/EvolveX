...
class GepaAnalyze(AnalyzeOperator):
    def analyze(self, checkout: Path, ctx: OperatorContext) -> AnalyzeResult:
        ...
        root = ctx.run_dir / "analyze" / "evidence"
        ...
        for index, (name, paths) in enumerate(components.items()):
            ...
            component_artifacts.append(f"analyze/evidence/{relative.as_posix()}")
        ...
        (ctx.run_dir / "analyze" / "feedback.md").write_text(selected)
        artifacts = [
            "analyze/feedback.md",
            "analyze/evidence/manifest.json",
            "analyze/evidence/reflective_dataset.json",
            "analyze/evidence/raw_traces.json",
