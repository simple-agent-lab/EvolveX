...
from evolve.mutate_budget import (
    harbor_mutate_budget,
...
_VISIBLE_RUN_INPUTS = ("rollout", "analyze", "feedback")
...
def _runs_ignore(runs_root: Path):
    ...
    def ignore(directory: str, names: list[str]) -> set[str]:
        ...
        try:
            ...
            return ignored
        if len(relative.parts) >= 3 and relative.parts[-2:] == ("mutate", "harbor"):
...
def _initialize_sanitized_git(workspace: Path) -> None:
    ...
    git(workspace, "config", "user.name", "RSIHub Mutate Operator")
    git(workspace, "config", "user.email", "mutate@rsihub.invalid")
...
def _copy_visible_generation_inputs(source: Path, destination: Path) -> None:
    for name in _VISIBLE_RUN_INPUTS:
        ...
        if subtree.exists():
            ...
            ignore = shutil.ignore_patterns("judge") if name == "analyze" else None
...
def _agent_env(config: dict[str, Any]) -> dict[str, str]:
    ...
    # explicit mutate.agent_env mapping.
    if agent != "codex":
...
def run_agent(checkout: Path, prompt: str, ctx: OperatorContext) -> AgentRunResult:
    ...
    harbor_root = ctx.run_dir / "mutate" / "harbor"
