    ...
    normalized.update(
        {
            ...
            "max_examples": positive_int(config, "max_examples", 10),
            "required_placeholders": string_list(config, "required_placeholders", []),
...
def build_prompt(checkout: Path, ctx: OperatorContext) -> tuple[str, dict[str, Any]]:
    ...
    evidence_root = ctx.run_dir / "analyze" / "evidence"
...
class GepaMutate(MutateOperator):
...
if __name__ == "__main__":
    sdk.main(GepaMutate, validate_config=validate_config)
