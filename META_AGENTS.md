...
The Harbor runner copies `target/` into a generated Harbor Exec task at
`/app/target`. The selected Harbor agent edits that isolated copy. Harbor then
collects `/app/target` as an artifact; the operator requires a successful trial
and artifact manifest, rejects symlinks and special files, stages the returned
tree, and atomically replaces the child checkout's `target/`. Normal Git diff,
surface, validation, evaluation, and gate checks still run afterward.
